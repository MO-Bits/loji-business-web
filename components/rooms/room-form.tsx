        ...
        <Stack direction="row" spacing={1.5} sx={{ alignItems: "center" }}>
          ...
          <Box>
            <Typography variant="h4">
              {roomId ? `Edit ${room?.name ?? "room"}` : "Add new room"}
            </Typography>
            <Typography color="text.secondary">
              Room details, capacity, price, amenities and images.
            </Typography>
          </Box>
        </Stack>
            ...
            <Stack direction="row" sx={{ flexWrap: "wrap", gap: 1 }}>
              {roomAmenities.map((item) => (
                <Chip
                  ...
                  label={item}
                  clickable
                  color={amenities.includes(item) ? "primary" : "default"}
                  variant={amenities.includes(item) ? "filled" : "outlined"}
                  onClick={() =>
                    ...
                    onClick={() =>
                      index < existingImages.length
                        ...
                        : setFiles((current) =>
                            current.filter(
                              (_, item) =>
                                item !== index - existingImages.length,
                            ),
                          )
                    }
                    sx={{
