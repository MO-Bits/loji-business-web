...
export function RoomsScreen() {
  ...
  return <Container maxWidth="xl" sx={{ py: { xs: 3, md: 4.5 } }}><Stack spacing={{ xs: 2.5, md: 3.5 }}>
    ...
    {!loading && rooms.length > 0 && <Stack direction="row" spacing={1}><Chip label={`${rooms.length} total rooms`} variant="outlined" /><Chip label={`${active} active`} color="success" variant="outlined" />{rooms.length - active > 0 && <Chip label={`${rooms.length - active} inactive`} variant="outlined" />}</Stack>}
    {loading ? <Box sx={{ display: "grid", gap: 2.5, gridTemplateColumns: { xs: "1fr", sm: "repeat(2,1fr)", xl: "repeat(3,1fr)" } }}>{[0,1,2,3,4,5].map((item) => <Skeleton key={item} height={350} variant="rounded" />)}</Box> : error ? <Alert severity="error" action={<Button color="inherit" startIcon={<RefreshRoundedIcon />} onClick={() => void refresh()}>Retry</Button>}>{error}</Alert> : rooms.length === 0 ? <EmptyRooms canManage={canManage} /> : <Box sx={{ display: "grid", gap: 2.5, gridTemplateColumns: { xs: "1fr", sm: "repeat(2,minmax(0,1fr))", xl: "repeat(3,minmax(0,1fr))" } }}>{rooms.map((room) => <RoomCard key={room.id} room={room} />)}</Box>}
  </Stack></Container>;
}
