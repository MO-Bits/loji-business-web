...
export function RoomsScreen() {
  ...
  return <Container maxWidth="lg" sx={{ py: { xs: 3, md: 5 } }}><Stack spacing={3}>
    <Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between" }}><Box><Typography variant="h4">Rooms</Typography><Typography color="text.secondary">Manage room inventory, prices and availability.</Typography></Box>{canManage && <Button component={Link} href="/rooms/new" variant="contained" startIcon={<AddRoundedIcon />}>Add room</Button>}</Stack>
...
function RoomCard({ room }: { room: Room }) {
  return <Paper component={Link} href={`/rooms/${room.id}`} variant="outlined" sx={{ color: "inherit", display: "grid", gridTemplateColumns: { xs: "110px 1fr", sm: "180px 1fr" }, overflow: "hidden", textDecoration: "none", transition: "150ms", "&:hover": { borderColor: "primary.main", transform: "translateY(-1px)" } }}>
