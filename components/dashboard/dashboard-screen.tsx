...
export function DashboardScreen() {
  ...
  return <Box sx={{ minHeight: "100dvh", pb: 6 }}>
    <Container maxWidth="xl" sx={{ pt: { xs: 2.25, sm: 3, lg: 4 } }}><Stack spacing={{ xs: 2.25, sm: 3, lg: 3.5 }}>
      <Paper sx={{ borderRadius: 1, color: "white", minHeight: { xs: 210, md: 230 }, overflow: "hidden", position: "relative", background: cover ? `linear-gradient(90deg, rgba(5,36,78,.94), rgba(5,36,78,.72)), url(${cover}) center/cover` : "linear-gradient(135deg, #073E86 0%, #0B66D4 62%, #2D8BE8 100%)" }}>
...
function MetricCard({ icon, label, value, detail, tone }: { icon: React.ReactNode; label: string; value: string | number; detail: string; tone: string }) {
  return <Paper variant="outlined" sx={{ borderRadius: 1, p: { xs: 1.75, sm: 2.25, lg: 2.5 } }}><Stack direction="row" spacing={1.5} alignItems="center"><Box sx={{ bgcolor: `${tone}14`, borderRadius: 1, color: tone, display: "grid", height: { xs: 40, sm: 44 }, placeItems: "center", width: { xs: 40, sm: 44 }, flexShrink: 0 }}>{icon}</Box><Box sx={{ minWidth: 0 }}><Typography sx={{ fontSize: { xs: "1.35rem", md: "1.7rem" }, fontWeight: 700, letterSpacing: "-.04em", lineHeight: 1 }}>{value}</Typography><Typography variant="body2" sx={{ fontWeight: 700, mt: .55 }}>{label}</Typography><Typography color="text.secondary" variant="caption">{detail}</Typography></Box></Stack></Paper>;
}
...
function StaySection({ title, subtitle, icon, tone, bookings, empty, compact = false }: { title: string; subtitle: string; icon: React.ReactNode; tone: string; bookings: DashboardBooking[]; empty: string; compact?: boolean }) {
  ...
  return <Paper variant="outlined" sx={{ borderRadius: 1, p: { xs: 2.25, sm: 3 } }}><SectionHeading title={title} subtitle={subtitle} icon={icon} tone={tone} count={bookings.length} /><Divider sx={{ my: 2.25 }} />{bookings.length === 0 ? <EmptyState text={empty} /> : <Stack spacing={.75}>{visible.map((booking) => <BookingTile key={booking.id} booking={booking} tone={tone} />)}{bookings.length > limit && <Button onClick={() => setShowAll((value) => !value)} endIcon={<ArrowForwardRoundedIcon />} sx={{ alignSelf: "flex-start" }}>{showAll ? "Show fewer" : `View all ${bookings.length}`}</Button>}</Stack>}</Paper>;
}

function BookingTile({ booking, tone }: { booking: DashboardBooking; tone: string }) {
  return <Box component={Link} href={`/bookings/${booking.id}`} sx={{ borderRadius: 1, color: "inherit", display: "block", p: 1.25, textDecoration: "none", transition: "background 160ms ease", "&:hover": { bgcolor: "action.hover" } }}><Stack direction="row" spacing={1.4} alignItems="center"><Avatar sx={{ bgcolor: `${tone}16`, color: tone, fontSize: ".8rem", height: 40, width: 40 }}>{initials(booking.guestName)}</Avatar><Box sx={{ flex: 1, minWidth: 0 }}><Typography noWrap sx={{ fontWeight: 700 }}>{booking.guestName}</Typography><Typography color="text.secondary" noWrap variant="caption">{booking.roomName}{booking.roomType ? ` · ${booking.roomType}` : ""}</Typography></Box><Box sx={{ textAlign: "right" }}><Typography variant="caption" sx={{ display: "block", fontWeight: 700 }}>{formatLocalDate(booking.checkOut, { day: "numeric", month: "short" })}</Typography><Chip label={statusLabel(booking.status)} size="small" sx={{ mt: .35, height: 22, fontSize: ".66rem" }} /></Box></Stack></Box>;
}

function FinanceCard({ revenue, outstanding }: { revenue: number; outstanding: number }) {
  return <Paper sx={{ background: "linear-gradient(145deg,#102A43,#173E65)", borderRadius: 1, color: "white", p: 3 }}><Stack direction="row" justifyContent="space-between"><Box><Typography variant="overline" sx={{ color: "rgba(255,255,255,.66)", letterSpacing: ".12em" }}>TODAY’S REVENUE</Typography><Typography variant="h4" sx={{ color: "white", mt: .7 }}>{money.format(revenue)}</Typography></Box><Box sx={{ bgcolor: "rgba(255,255,255,.1)", borderRadius: 1, display: "grid", height: 44, placeItems: "center", width: 44 }}><PaymentsRoundedIcon /></Box></Stack><Divider sx={{ borderColor: "rgba(255,255,255,.14)", my: 2.5 }} /><Stack direction="row" justifyContent="space-between"><Typography sx={{ color: "rgba(255,255,255,.72)" }}>Outstanding balance</Typography><Typography sx={{ fontWeight: 700 }}>{money.format(outstanding)}</Typography></Stack></Paper>;
}

function AvailableRooms({ rooms }: { rooms: DashboardRoom[] }) {
  return <Paper variant="outlined" sx={{ borderRadius: 1, p: { xs: 2.25, sm: 3 } }}><SectionHeading title="Available rooms" subtitle="Ready to sell tonight" icon={<HotelRoundedIcon />} tone="#0B66D4" count={rooms.length} /><Divider sx={{ my: 2.25 }} />{rooms.length === 0 ? <EmptyState text="No available rooms" /> : <Stack spacing={1.25}>{rooms.slice(0, 5).map((room) => <RoomCard key={room.id} room={room} />)}{rooms.length > 5 && <Button component={Link} href="/rooms">View all rooms</Button>}</Stack>}</Paper>;
}

function RoomCard({ room }: { room: DashboardRoom }) {
  return <Box component={Link} href={`/rooms/${room.id}`} sx={{ border: "1px solid", borderColor: "divider", borderRadius: 1, color: "inherit", display: "grid", gridTemplateColumns: room.images[0] ? "76px 1fr" : "1fr", overflow: "hidden", textDecoration: "none", transition: "border-color 160ms ease", "&:hover": { borderColor: "primary.main" } }}>{room.images[0] && <Box component="img" src={room.images[0]} alt={room.name} sx={{ height: "100%", minHeight: 86, objectFit: "cover", width: 76 }} />}<Stack spacing={.35} sx={{ minWidth: 0, p: 1.25 }}><Typography noWrap sx={{ fontWeight: 700 }}>{room.name}</Typography><Typography color="text.secondary" variant="caption" sx={{ textTransform: "capitalize" }}>{room.roomType} · {room.capacity} guests</Typography><Typography color="primary" variant="body2" sx={{ fontWeight: 700 }}>{money.format(room.pricePerNight)} <Typography component="span" color="text.secondary" variant="caption">/ night</Typography></Typography></Stack></Box>;
}

function EmptyState({ text }: { text: string }) { return <Box sx={{ bgcolor: "#F7F9FB", border: "1px dashed", borderColor: "divider", borderRadius: 1, p: 3, textAlign: "center" }}><Typography color="text.secondary" variant="body2">{text}</Typography></Box>; }
