...
function WizardActions({ activeStep, busy, checkInNow, onBack }: { activeStep: number; busy: boolean; checkInNow: boolean; onBack: () => void }) {
  const { t } = useLanguage();
  return (
    <Paper elevation={0} sx={{ backdropFilter: { xs: "blur(18px)", md: "none" }, bgcolor: { xs: "color-mix(in srgb, var(--mui-palette-background-paper) 94%, transparent)", md: "background.paper" }, border: { xs: 1, md: 0 }, borderColor: "divider", borderRadius: { xs: 3, md: 1 }, bottom: { xs: "calc(78px + env(safe-area-inset-bottom))", md: "auto" }, boxShadow: { xs: "0 12px 38px rgba(15, 34, 58, .16)", md: "none" }, left: { xs: 10, md: "auto" }, p: { xs: 1, md: 1.5 }, position: { xs: "fixed", md: "static" }, right: { xs: 10, md: "auto" }, zIndex: { xs: (theme) => theme.zIndex.appBar - 1, md: "auto" } }}>
