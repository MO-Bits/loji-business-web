...
export function BookingsScreen() {
  ...
  return <Container maxWidth="xl" sx={{ py: { xs: 2.5, sm: 3.25, lg: 4.5 } }}><Stack spacing={{ xs: 2.25, sm: 3, lg: 3.5 }}>
    ...
    <Paper variant="outlined" sx={{ borderRadius: 3.5, p: { xs: 1.75, sm: 2.5 } }}><Stack direction={{ xs: "column", md: "row" }} spacing={1.5}>
...
function Stat({ icon, label, value, tone }: { icon: React.ReactNode; label: string; value: number; tone: string }) {
  return <Paper variant="outlined" sx={{ borderRadius: 3.5, p: { xs: 1.5, sm: 2 } }}><Stack direction="row" spacing={1.25} alignItems="center"><Box sx={{ bgcolor: `${tone}14`, borderRadius: 2.25, color: tone, display: "grid", flexShrink: 0, height: 40, placeItems: "center", width: 40 }}>{icon}</Box><Box><Typography sx={{ fontSize: { xs: "1.3rem", sm: "1.5rem" }, fontWeight: 820, letterSpacing: "-.04em", lineHeight: 1 }}>{value}</Typography><Typography color="text.secondary" variant="caption">{label}</Typography></Box></Stack></Paper>;
}
...
function BookingRow({ booking, last }: { booking: Booking; last: boolean }) {
  ...
  return <Box component={Link} href={`/bookings/${booking.id}`} sx={{ borderBottom: last ? 0 : "1px solid", borderColor: "divider", color: "inherit", display: { xs: "block", lg: "grid" }, gap: 2, gridTemplateColumns: { lg: "minmax(210px,1.3fr) minmax(140px,.8fr) minmax(180px,1fr) 120px 140px" }, px: { xs: 1.5, sm: 2, lg: 2.5 }, py: { xs: 1.75, lg: 1.6 }, textDecoration: "none", transition: "background 150ms ease", "&:hover": { bgcolor: "#F8FAFC" } }}>
    <Stack direction="row" spacing={1.25} alignItems="center"><Avatar sx={{ bgcolor: "#EAF3FF", color: "primary.dark", flexShrink: 0, fontSize: ".78rem", height: 40, width: 40 }}>{initials || "G"}</Avatar><Box sx={{ minWidth: 0 }}><Typography noWrap sx={{ fontWeight: 780 }}>{booking.guestName}</Typography><Typography color="text.secondary" noWrap variant="caption">{booking.bookingNumber} · {booking.phone || "No phone"}</Typography></Box><Chip label={bookingStatusLabel(booking.status)} color={statusColor(booking.status)} size="small" sx={{ display: { xs: "inline-flex", lg: "none" }, ml: "auto!important", maxWidth: 110 }} /></Stack>
