...
export function BookingsScreen() {
  ...
  return <Container maxWidth="lg" sx={{ py: { xs: 3, md: 5 } }}><Stack spacing={3}><Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ alignItems: { sm: "center" }, justifyContent: "space-between" }}><Box><Typography variant="h4">Bookings</Typography><Typography color="text.secondary">Reservations, arrivals, guests and departures.</Typography></Box><Button component={Link} href="/bookings/new" variant="contained" startIcon={<AddRoundedIcon />}>New booking</Button></Stack>
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: number }) { return <Paper variant="outlined" sx={{ p: { xs: 1.5, sm: 2 } }}><Stack direction="row" spacing={1.2} sx={{ alignItems: "center" }}><Box sx={{ color: "primary.main", display: { xs: "none", sm: "block" } }}>{icon}</Box><Box><Typography sx={{ fontSize: "1.35rem", fontWeight: 850 }}>{value}</Typography><Typography color="text.secondary" variant="caption">{label}</Typography></Box></Stack></Paper>; }
function BookingRow({ booking }: { booking: Booking }) { const initials = booking.guestName.split(/\s+/).slice(0,2).map((part) => part[0]).join("").toUpperCase(); return <Paper component={Link} href={`/bookings/${booking.id}`} variant="outlined" sx={{ color: "inherit", display: "block", p: 2, textDecoration: "none", "&:hover": { borderColor: "primary.main" } }}><Stack direction="row" spacing={1.5} sx={{ alignItems: "center" }}><Avatar sx={{ bgcolor: "primary.main" }}>{initials || "G"}</Avatar><Box sx={{ flex: 1, minWidth: 0 }}><Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between" }}><Typography noWrap sx={{ fontWeight: 800 }}>{booking.guestName}</Typography><Chip label={bookingStatusLabel(booking.status)} color={statusColor(booking.status)} size="small" /></Stack><Typography color="text.secondary" noWrap variant="body2">{booking.bookingNumber} · {booking.roomName}</Typography><Typography color="text.secondary" variant="caption">{date.format(booking.checkIn)} → {date.format(booking.checkOut)} · {booking.totalGuests} guests</Typography></Box></Stack></Paper>; }
