...
export function BookingsScreen() {
  ...
  const today = localDateKey(); const arrivals = bookings.filter((item) => localDateKey(item.checkIn) === today && item.status !== "cancelled").length; const departures = bookings.filter((item) => localDateKey(item.checkOut) === today && item.status !== "cancelled").length; const staying = bookings.filter((item) => item.status === "checked_in").reduce((sum, item) => sum + item.totalGuests, 0);
