...
export function AccountScreen() {
  ...
  return <Container maxWidth="sm" sx={{ py: { xs: 3, md: 5 } }}><Stack spacing={3}><Button color="inherit" startIcon={<ArrowBackRoundedIcon />} onClick={() => router.back()} sx={{ alignSelf: "flex-start" }}>Back</Button><Stack sx={{ alignItems: "center" }}><Avatar src={account.avatar_url ?? undefined} sx={{ bgcolor: "primary.main", boxShadow: "0 10px 28px rgba(23,105,210,.2)", fontSize: "2rem", height: 90, width: 90 }}>{name[0]?.toUpperCase()}</Avatar><Typography variant="h4" sx={{ mt: 2 }}>{name}</Typography><Typography color="text.secondary" sx={{ textTransform: "capitalize" }}>{String(account.role ?? "Member")}</Typography></Stack><InfoCard title="Personal Details" rows={[[<EmailRoundedIcon key="email" />, "Email", account.email], [<PhoneRoundedIcon key="phone" />, "Phone", account.phone]]} /><InfoCard title="Property Membership" rows={[[<ShieldRoundedIcon key="role" />, "Role", account.role], [<ShieldRoundedIcon key="status" />, "Status", account.status]]} />
    <Box sx={{ display: { xs: "block", md: "none" } }}><Typography color="text.secondary" variant="overline" sx={{ mb: 1, px: 1 }}>Workspace settings</Typography><Paper variant="outlined"><List disablePadding>{canManage && <><MobileLink href="/more/property" icon={<ApartmentRoundedIcon />} title="Property" subtitle="Details, amenities and photos" /><Divider /><MobileLink href="/more/staff" icon={<GroupsRoundedIcon />} title="Staff" subtitle="Users, invitations and permissions" /><Divider /></>}<ListItemButton onClick={() => void signOut()} sx={{ color: "error.main", py: 1.5 }}><ListItemIcon sx={{ color: "error.main", minWidth: 40 }}><LogoutRoundedIcon /></ListItemIcon><ListItemText primary="Sign out" slotProps={{ primary: { sx: { fontWeight: 600 } } }} /></ListItemButton></List></Paper></Box>
  </Stack></Container>;
}
...
function InfoCard({ title, rows }: { title: string; rows: [React.ReactNode, string, unknown][] }) { return <Box><Typography color="text.secondary" variant="overline" sx={{ mb: 1, px: 1 }}>{title}</Typography><Paper variant="outlined">{rows.map(([icon, label, value], index) => <Box key={label}>{index > 0 && <Divider />}<Stack direction="row" spacing={1.5} sx={{ alignItems: "center", p: 2 }}>{icon}<Typography sx={{ fontWeight: 500 }}>{label}</Typography><Typography color="text.secondary" sx={{ ml: "auto!important", textTransform: label === "Role" || label === "Status" ? "capitalize" : "none" }}>{String(value ?? "-")}</Typography></Stack></Box>)}</Paper></Box>; }
function MobileLink({ href, icon, title, subtitle }: { href: string; icon: React.ReactNode; title: string; subtitle: string }) { return <ListItemButton component={Link} href={href} sx={{ py: 1.5 }}><ListItemIcon sx={{ color: "primary.main", minWidth: 40 }}>{icon}</ListItemIcon><ListItemText primary={title} secondary={subtitle} slotProps={{ primary: { sx: { fontWeight: 600 } } }} /><ChevronRightRoundedIcon color="action" /></ListItemButton>; }
