...
function OperationsForm({ client, propertyId, workspace }: { client: ReturnType<typeof usePropertySettings>["client"]; propertyId: string; workspace: PropertySettingsWorkspace }) {
  ...
  return (
    <Stack component="form" onSubmit={(event) => void submit(event)} spacing={{ xs: 2.5, sm: 3 }}>
      ...
      <SettingsSection description={t("These are operational defaults; each booking still keeps its own stay dates.", "Hizi ni saa za kawaida za uendeshaji; kila uhifadhi bado una tarehe zake za ukaaji.")} title={t("Guest arrival and departure", "Kuingia na kutoka kwa wageni")}>
