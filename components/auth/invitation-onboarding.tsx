...
export function InvitationOnboarding() {
  ...
  async function loadInvitation() {
    ...
    try {
      const { data, error } = await supabase.rpc(
        ...
        { p_token: normalizedToken } as never,
      );
  ...
  return (
    <Box
      component="main"
      sx={{ minHeight: "100dvh", display: "grid", placeItems: "center", py: 4 }}
    >
          ...
          <Paper variant="outlined" sx={{ p: { xs: 2.5, sm: 4 } }}>
            <Stack spacing={3}>
              <Alert icon={<InfoRoundedIcon />} severity="info">
                ...
                <br />
                Ask the staff member who invited you for the 8-character code.
              </Alert>
              <TextField
                label="Invitation code"
                value={token}
                onChange={(event) =>
                  setToken(event.target.value.toUpperCase().slice(0, 9))
                }
      ...
      >
          ...
          <Stack spacing={2.5}>
            ...
            >
              ...
              <Typography sx={{ opacity: 0.86, mt: 0.75 }}>
                {invitation?.formatted_address || "Address unavailable"}
              </Typography>
            </Paper>
        ...
        <DialogActions sx={{ p: 3, pt: 1 }}>
          ...
          <Button
            variant="contained"
            disabled={loading}
            onClick={() => void respond("accept_property_invitation")}
          >
            {loading ? (
