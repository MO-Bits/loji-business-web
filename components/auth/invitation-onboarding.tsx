...
export function InvitationOnboarding() {
  ...
  return <Box component="main" sx={{ minHeight: "100dvh", display: "grid", placeItems: "center", py: 4 }}><Container maxWidth="sm"><Stack spacing={3}>
    ...
    <Paper variant="outlined" sx={{ p: { xs: 2.5, sm: 4 }, borderRadius: 4 }}><Stack spacing={3}>
      ...
      <TextField label="Invitation code" value={token} onChange={(event) => setToken(event.target.value.toUpperCase().slice(0, 9))} placeholder="XXXX-XXXX" autoComplete="off" slotProps={{ input: { startAdornment: <InputAdornment position="start"><ConfirmationNumberRoundedIcon color="primary" /></InputAdornment> }, htmlInput: { style: { textAlign: "center", letterSpacing: 5, fontWeight: 700, textTransform: "uppercase" } } }} />
  ...
  <Dialog open={Boolean(invitation)} onClose={loading ? undefined : () => setInvitation(null)} fullWidth maxWidth="xs"><DialogTitle>Property invitation</DialogTitle><DialogContent><Stack spacing={2.5}>
    <Paper elevation={0} sx={{ p: 3, borderRadius: 1, bgcolor: "primary.main", color: "primary.contrastText" }}><BusinessRoundedIcon sx={{ fontSize: 38 }} /><Typography variant="h5" fontWeight={700} sx={{ mt: 1 }}>{invitation?.property_name || "Property"}</Typography><Typography sx={{ opacity: .86, mt: .75 }}>{invitation?.formatted_address || "Address unavailable"}</Typography></Paper>
    <Typography color="text.secondary">You have been invited as</Typography><Paper variant="outlined" sx={{ p: 2 }}><Stack direction="row" spacing={1.5} alignItems="center"><PersonRoundedIcon color="primary" /><Typography fontWeight={700}>{(invitation?.role || "Staff").toUpperCase()}</Typography></Stack></Paper>
