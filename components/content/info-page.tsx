...
function LegalSection({ title, content }: LegalSectionData) {
  return <Paper component="section" variant="outlined" sx={{ p: { xs: 2.5, sm: 3 }, borderRadius: 3 }}>
    <Stack direction="row" spacing={1.5} alignItems="flex-start">
      <Box sx={{ width: 4, height: 20, mt: .25, borderRadius: 1, bgcolor: "primary.main", flexShrink: 0 }} />
...
export function LegalPage({ kind, title, introTitle, intro, sections }: { kind: "terms" | "privacy"; title: string; introTitle: string; intro: string; sections: LegalSectionData[] }) {
  ...
  return <Box component="main" sx={{ minHeight: "100dvh", py: { xs: 2, md: 5 } }}><Container maxWidth="md"><Stack spacing={3}>
    <Stack direction="row" alignItems="center" spacing={1.5}><Button component={Link} href="/login" startIcon={<ArrowBackRoundedIcon />}>Back</Button><Divider orientation="vertical" flexItem /><Typography variant="h5" fontWeight={700}>{title}</Typography></Stack>
    <Paper variant="outlined" sx={(theme) => ({ p: { xs: 2.5, sm: 3.5 }, borderRadius: 1, background: `linear-gradient(135deg, ${alpha(theme.palette.primary.main, .12)}, ${alpha(theme.palette.background.paper, .95)})` })}>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2.5} alignItems={{ sm: "center" }}><Box sx={{ width: 54, height: 54, borderRadius: 1, display: "grid", placeItems: "center", bgcolor: "primary.main", color: "primary.contrastText", flexShrink: 0 }}><Icon /></Box><Box><Typography variant="h6" color="primary.main" fontWeight={700}>{introTitle}</Typography><Typography color="text.secondary" sx={{ mt: .75, lineHeight: 1.65 }}>{intro}</Typography><Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 1.5 }}><CalendarMonthRoundedIcon color="primary" sx={{ fontSize: 17 }} /><Typography variant="caption" fontWeight={700}>Effective Date: August 7, 2026</Typography></Stack></Box></Stack>
    </Paper>
