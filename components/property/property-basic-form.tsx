...
export function PropertyBasicForm() {
  ...
  const pickFiles = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length + picked.length > MAX_PROPERTY_IMAGES)
      return setLocalError("You can upload up to 3 photos.");
    const tooLarge = picked.find(
      (file) => file.size > MAX_PROPERTY_IMAGE_BYTES,
    );
  ...
  const submit = async () => {
    if (!name.trim() || !phone.trim())
      return setLocalError("Property name and phone are required.");
    ...
    try {
      ...
      /* controller exposes the message */
    }
  };
      ...
      <Container maxWidth="md">
        <Stack spacing={{ xs: 2.5, sm: 3.5 }}>
          ...
          <Stack direction="row" spacing={2} sx={{ alignItems: "center" }}>
            ...
            <Box>
              <Typography variant="h4">Create your property profile</Typography>
              <Typography color="text.secondary">
                Property details · Step 2 of 3
              </Typography>
            </Box>
          </Stack>
              ...
              >
                ...
                {files.length < 3 && (
                  <Button
                    ...
                    sx={{ aspectRatio: "4 / 3", borderStyle: "dashed" }}
                  >
                    Add photos
                    <input
                      hidden
      ...
      >
        <Container maxWidth="md">
          <Button
            ...
            onClick={submit}
          >
            {controller.loading ? "Creating your property…" : "Create property"}
          </Button>
        </Container>
      </Paper>
