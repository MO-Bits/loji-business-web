...
const PROPERTY_LOCATION_DRAFT_PREFIX =
  "loji-property-location-registration:v1";
...
export function propertyLocationDraftKey(
  ownerId: string,
  propertyId: string,
) {
  return `${PROPERTY_LOCATION_DRAFT_PREFIX}:${ownerId}:${propertyId}`;
}

export function clearPropertyLocationDraft(
  ownerId: string,
  propertyId: string,
) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.removeItem(
      propertyLocationDraftKey(ownerId, propertyId),
    );
  } catch {
    // Browser storage is only a resume convenience.
  }
